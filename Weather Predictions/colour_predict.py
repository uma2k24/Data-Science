...
def main():
    ...
    function_transformer_LAB = FunctionTransformer(rgb2lab)
    function_transformer_HSV = FunctionTransformer(rgb2hsv)

    bayes_rgb_model = make_pipeline(GaussianNB())
    bayes_convert_model = make_pipeline(function_transformer_HSV, GaussianNB())

    knn_rgb_model = make_pipeline(KNeighborsClassifier(n_neighbors=(len(pd.unique(y))-3)))
    knn_convert_model = make_pipeline(function_transformer_LAB, KNeighborsClassifier(n_neighbors=(len(pd.unique(y))-3)))

    rf_rgb_model = make_pipeline(RandomForestClassifier(n_estimators=100, criterion='gini'))
    rf_convert_model = make_pipeline(function_transformer_LAB, RandomForestClassifier(n_estimators=100, criterion='gini'))
